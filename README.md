...
> 注意: 修改配置文件最好使用 Notepad++ 等专业文本编辑器, 不要使用记事本, 不要使用记事本, 不要使用记事本, 
重要的事情说三遍, 因为项目文件均为兼容性最好的编码 UTF-8 作为保存格式, 但在保存 UTF-8 格式文件
时 Windows 偏要加 BOM, 这是 Windows 的工程师自作聪明, 借此吐槽之余重申一遍, 不要使用记事本
